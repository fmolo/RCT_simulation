...
This notebook describes and runs the simulations in Section 5.3 of [cite preprint] (discussing @Worrall:2002aa) using the R statistical programming language.
...
Then we run the simulation `{r} format(r, scientifc = FALSE)` times for each number of potential confounders $p$. We set a 'seed' to be able to always reproduce the same pseudorandom data. The results of all simulations are stored in a list and saved on disk. 
...
## More analyses of the simulation output

We perform some additional analyses of the simulation output that are not included in [cite_preprint]. The *empirical bias* of an estimator is defined as the difference between the estimated and the true effect across simulations. The *empirical variance* of an estimator quantifies the variability of the effect size estimates across simulations. Added together, the squared bias and the variance of an estimator constitute the estimator's *Mean Squared Error*.

We calculate the empirical bias and the empirical variance of the linear regression estimator after randomization across our `{r} format(r, scientific = FALSE)` simulations. We plot these quantities as a function of the number of potential confounders $p$. The imprecision of these estimates due to the finite number of simulations is called the Monte Carlo error; we calculate it according to Table 3 in @Pawel:2024aa.

We let the number of potential confounders increase to a maximum 10000, but only for $p$ a power of ten.
```{r}
...
The empirical bias is close to zero for small as well as for large numbers of confounders, i.e. on average the estimated effect size corresponds to the true effect size. This is the case even when there are many more potential confounders $p$ than observations $n$. Note that in a trial with more potential confounders, the uncertainty in estimating the bias increases. This is a result of the increasing variance of the estimator for increased $p$ (cf. next section).
...
As the number of potential confounders increases, the variance of the estimator also increases, i.e. the estimated effect sizes across simulations show a higher variability.
...
How often does the estimated 95% confidence interval cover the true value of the effect? This does not depend on the number of potential confounders: The empirical coverage across simulations is close to the confidence level of 95%.
...
Record the version of R and all involved R packages for reproducibility:
